            ...
            >
                <div class="minePageClass">
                    <div
                        ...
                        v-if="(!($config.isLogined()) || !isAuth) && $config.userFromType() == 'app' "
                    ...
                    >
                        ...
                        <div class="infoDetail">
                            ...
                            <div
                                class="info-name"
                                v-else
                            >{{userInfo.account | accountHide}}</div>
                    ...
                    <div class="listBoxClass">
                        ...
                        >
                            <div class="listcell-main">
                                ...
                                <div class="listcell-main-right">
                                    <!-- 版本 -->
                                    <span class="listcell-main-right-txt">2.1.3</span>
                                    <img
                                        src="static/img/warning/arrow@2x.png"
                                        class='select-row-arrow'
                                    />
...
import { getInfoFromSessionStorage, goAppLogin, getPlatformCode, isWechatPlatform, isWechatBind, cookieForFirstShowNotice, getAuthStatu, newUserFromType,getSearchParams } from "@public/js/util.js";
...
export default {
    ...
    created() {
        const t = this;
        const showChangePlatformPlane = getSearchParams(globalConsts.urlParamsKeys.showChangePlatformPlane.name)
        if(showChangePlatformPlane && isWechatBind()){
    ...
    methods: {
        createAlert(bindNotice) {
            let that = this
            if (bindNotice && !isWechatPlatform()) {
                // 显示通知弹窗
        ...
        getWechatStatu() {
            request.isWechatEntyShow()
                .then(({ wx_show = "", wx_bind = "" }) => {
                    ...
                    if (wx_bind == 1) {
                        this.bindNotice = true;
                    } else {
                        this.bindNotice = false;
                    }
                    this.createAlert(this.bindNotice)
                })
                .catch(e => {
                    commonError(e, this);
                });
        ...
        toWarningNotice() {
            //  console.log(`$config.isLogined():${this.$config.isLogined()}`,`newUserFromType()=${this.newUserFromType()}`,`isAuth:${this.isAuth}`,`this.$config.userFromType():${this.$config.userFromType()}`)
