...
import { commonError, saveInfoToSessionStorage, getInfoFromSessionStorage, getPlatformCode, saveInfoToStorage, iConsole, initPage,removeStorage,wechatAuthDirect } from '@public/js/util.js'
...
export default {
    ...
    methods: {
        ...
        errorDirectToIndex() {
            clearLocalData()
            location.href = "http://iaitrader.bbyou.net/iai_trader/warning/warning.html#/index?platform=PF00&platformName=wechat&platforVersion=2&goPage=7&tabIndex=2"
        },
        // openid为空，处理方法
        openidEmpty() {
            this.$createDialog({
                type: 'confirm',
                title: '提示',
                content: '您的微信授权码因技术原因，被外星人劫持啦。请点击“重新授权“来修复此问题。',
                confirmBtn: {
                    text: '重新授权',
                    active: true,
                    disabled: false,
        ...
        wechatRegisterRequest() {
            ...
            request.wechatRegister(params).then((res) => {
                ...
                if (status == globalConsts.resStatus.success) {
                    ...
                    setTimeout(function () {
                        //////////////////////
                        initPage(t.$router, { goPage: t.refreshPage })
                    }, 2000)
...
.wechat-login-box-body {
    padding: 0 0.6rem 0.42rem 0.6rem;
}
.form-error-tip {
    font-weight: 400;
